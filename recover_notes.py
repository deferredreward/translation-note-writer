...
def parse_log_for_unwritten_notes(log_content: str) -> List[Dict[str, Any]]:
    ...
    Returns:
        ...
        with its context (editor_key, verse, row, note).
    """
    recovered_notes = []
    
    # This pattern finds the final, formatted note along with its direct context (verse and row).
    # It looks for the "Processing AI output" line followed by the "Final formatted note" line.
    final_note_pattern = re.compile(
        r"INFO - Processing AI output for (?P<verse>[\d:]+) \(row (?P<row>\d+)\):[^\n]*\n"
        r"(?:.*?\n)*?"  # Non-greedy match for any lines in between
        r"INFO - Final formatted note for (?P=verse): (?P<note>.*)"
    )

    # This pattern is used to find the editor associated with a batch.
    # We assume the last "Batch ... completed" log before a note determines the editor.
    editor_pattern = re.compile(r"INFO - Batch \S+ for .*?\((?P<editor_key>editor\d+)\) completed")

    for note_match in final_note_pattern.finditer(log_content):
        verse = note_match.group('verse')
        row = int(note_match.group('row'))
        note = note_match.group('note').strip()
        
        # To find the correct editor, we search backwards from the note's position in the log.
        content_before_note = log_content[:note_match.start()]
        
        editor_key = None
        # Find the *last* editor pattern match before our note. This is the most likely editor.
...
def main():
    ...
    for note_info in recovered_notes:
        ...
        try:
            ...
            # The user specified checking the "Go?" column for "AI"
            ...
            logger.debug(f"  -> Note: {note_text[:150]}...")

            if not args.dry_run:
                # Get column names from config
                note_col = config.get('sheets.column_names.note_column', 'Note')
                status_col = config.get('sheets.column_names.status_column', 'Status')

                # Prepare the data to write to the sheet
                updates = [
                    {'range': f"{note_col}{row}", 'values': [[note_text]]},
                    {'range': f"{status_col}{row}", 'values': [['AI Written']]},
                    {'range': f"{status_col_name}{row}", 'values': [['AI']]},
                ]
                
                sheet_manager.batch_update_cells(sheet_id, updates)
                logger.info(f"Successfully wrote note to row {row} for {friendly_name}.")
                written_count += 1
            else:
                logger.info(f"DRY RUN: Would write note to row {row} for {friendly_name}.")
                written_count += 1
