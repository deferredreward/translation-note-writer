...
class CacheManager:
    ...
    def get_cached_data(self, cache_type: str, user: str = None, book: str = None) -> Optional[Any]:
        ...
        try:
            ...
            # Validate biblical text cache integrity
            if cache_type.endswith('_chapters') and user and book:
                chapters = data.get('chapters', [])
                if chapters:
                    # Check for verse gaps in chapter 8 specifically (1 Kings issue)
                    if data.get('book') == '1KI':
                        ch8 = next((ch for ch in chapters if ch.get('chapter') == 8), None)
                        if ch8:
                            verse_nums = [v.get('number') for v in ch8.get('verses', [])]
                            # Check for suspicious gaps (like missing 41-42)
                            for i in range(len(verse_nums)-1):
                                if verse_nums[i+1] - verse_nums[i] > 1:
                                    gap_start, gap_end = verse_nums[i], verse_nums[i+1]
                                    if gap_end - gap_start > 1:  # More than 1 verse gap
                                        self.logger.warning(f"Cache validation: Gap found in {cache_file} chapter 8: verses {gap_start} -> {gap_end}")
