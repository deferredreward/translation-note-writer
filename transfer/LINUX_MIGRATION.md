...
### Step 5: Fix Windows Line Endings (Critical!)

**Files transferred from Windows may have Windows line endings (`\r\n`) that cause `\r command not found` errors on Linux.**

```bash
# Method 1: Using sed (works everywhere - RECOMMENDED)
sed -i 's/\r$//' run_translation_notes.sh
sed -i 's/\r$//' main.py
...
find . -name "*.yaml" -o -name "*.yml" -exec sed -i 's/\r$//' {} \;

# Method 2: Using dos2unix (if available)
sudo apt install dos2unix
dos2unix run_translation_notes.sh
dos2unix main.py
find . -name "*.py" -exec dos2unix {} \;

# Method 3: Using tr command
tr -d '\r' < run_translation_notes.sh > /tmp/fixed_script.sh
mv /tmp/fixed_script.sh run_translation_notes.sh

# Verify line endings are fixed:
file run_translation_notes.sh
...
# Create missing scripts directory if needed:
mkdir -p scripts
touch scripts/__init__.py

# Set proper permissions
chmod 755 main.py
chmod +x run_translation_notes.sh  # Make script executable
chmod -R 755 modules/
...
```bash
# Fix `\r command not found` errors:
sed -i 's/\r$//' run_translation_notes.sh
sed -i 's/\r$//' main.py

# Fix all files at once:
find . -type f \( -name "*.py" -o -name "*.sh" -o -name "*.yaml" \) -exec sed -i 's/\r$//' {} \;

# Check if files have Windows line endings:
file run_translation_notes.sh
...
mkdir -p scripts
touch scripts/__init__.py
echo "def extract_headwords(repo_path): return []" > scripts/extract_tw_headwords.py
```
...
**Cron job debugging:**
```bash
# Test cron script with debug output:
bash -x ./run_translation_notes.sh

# Add debug logging to cron:
*/30 6-20 * * 1-6 cd /path/to/project && bash -x ./run_translation_notes.sh >> logs/debug.log 2>&1

# Check if cron ran:
grep CRON /var/log/syslog | tail -10
tail -f logs/cron.log

# Exit systemctl/journalctl screens:
# Press 'q' to quit
# Or Ctrl+C to force exit
```

**Power management for teaching computer:**
```bash
# Check if suspend works without sudo:
systemctl suspend     # Test this
loginctl suspend      # Or this

# Cron schedule for teaching computer (6 AM - 8 PM, skip Sunday):
*/30 6-20 * * 1-6 /path/to/run_translation_notes.sh >/dev/null 2>&1
30 20 * * 1-6 systemctl suspend

# Monitor cron jobs without MTA warnings:
*/30 6-20 * * 1-6 /path/to/run_translation_notes.sh >/dev/null 2>&1
```
