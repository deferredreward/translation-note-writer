...
class TranslationNotesAI:
    ...
    def check_for_work(self):
        ...
        try:
            ...
            for editor_name, sheet_id in sheet_ids.items():
                # Check for shutdown before processing each sheet
                if getattr(self, '_shutdown_requested', False):
                    self.logger.info(f"Shutdown requested, stopping at {editor_name}")
                    break
                
                # Check if sheet is blocked due to permission errors
                if self._is_sheet_blocked(sheet_id, editor_name):
                    continue
                ...
                try:
                    ...
                    if support_references and auto_convert_sref:
                        self.logger.debug(f"Converting SRef values for {editor_name}...")
                        try:
                            ...
                            if all_items:
                                ...
                                if updates_needed:
                                    ...
                                    self.logger.debug(f"No SRef conversions needed for {editor_name}")
                        except Exception as e:
                            if self._is_permission_error(e):
                                self._block_sheet_for_permission_error(sheet_id, editor_name)
                                continue
                            else:
                    ...
                    if getattr(self, '_shutdown_requested', False):
                        self.logger.info(f"Shutdown requested during SRef conversion for {editor_name}")
                        break
                    ...
                    if pending_items:
                        ...
                        if getattr(self, '_shutdown_requested', False):
                            self.logger.info(f"Shutdown requested, skipping processing for {editor_name}")
                            break
                    ...
                    if self._is_permission_error(e):
                        ...
                        self.logger.error(f"Error processing sheet for {editor_name}: {e}")
                        self.handle_error(e, f"Processing sheet for {editor_name}")
            ...
            for user, sheet_id in sheet_ids.items():
                try:
                    ...
                    if pending_items:
                        ...
                        self.batch_processor.process_items_for_user(user, pending_items, dry_run=dry_run)
    ...
    def convert_sref_values(self):
        ...
        try:
            ...
            for editor_name, sheet_id in sheet_ids.items():
                try:
                    self.logger.info(f"Processing SRef conversion for {editor_name}...")
                    
                    # Get all rows from this sheet
                    all_items = self.sheet_manager.get_all_rows_for_sref_conversion(sheet_id)
                    
                    if not all_items:
                        self.logger.info(f"No rows found for {editor_name}")
                        continue
                    ...
                    if updates_needed:
                        self.logger.info(f"Found {len(updates_needed)} SRef conversions needed for {editor_name}")
                        ...
                        self.logger.info(f"No SRef conversions needed for {editor_name}")
                
                except Exception as e:
                    self.logger.error(f"Error processing SRef conversion for {editor_name}: {e}")
                    self.handle_error(e, f"SRef conversion for {editor_name}")
